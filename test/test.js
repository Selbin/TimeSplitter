...
describe('function to test time splitter', () => {
  it('verifying results', () => {
    const returnValue = []
    const result = [{
    ...
    for (let i = 0; i < startArr.length; i++) {
      returnValue.push(timeDivider(startArr[i], endArr[i]))
    }
    assert.strictEqual(JSON.stringify(returnValue), JSON.stringify(result))
  })
})
